...
export class AudioController {
    constructor() {
        ...
        // Check for precomputed data (WASM large file mode)
        this.precomputedData = DOMUtils.getPrecomputedData();
    ...
    async initPrecomputedMode() {
        ...
        const setupAfterReady = async () => {
            ...
            console.log('Setting up precomputed audio viewer...');

            // Setup spectrogram with precomputed data
            if (spectrogram && spectrogram.length > 0) {
                await this.pluginManager.setupSpectrogramPrecomputed(spectrogram, sampleRate);
            }
            ...
            // Update info
            this.fileInfoManager.updateDuration(duration);
            this.fileInfoManager.updateFileInfo();
        };

        // In precomputed mode, wavesurfer fires 'ready' when it has rendered the peaks
