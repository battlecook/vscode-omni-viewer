...
import { configureWebview, createReadonlyDocument, registerRefreshableViewer, renderErrorHtml, rerouteIfNeeded } from './viewerProviderUtils';

export class MatViewerProvider implements vscode.CustomReadonlyEditorProvider {
    public static readonly viewType = 'omni-viewer.matViewer';
    ...
    public async resolveCustomEditor(document: vscode.CustomDocument, webviewPanel: vscode.WebviewPanel): Promise<void> {
        ...
        registerRefreshableViewer(document.uri, MatViewerProvider.viewType, webviewPanel, async () => {
        ...
        try {
            if (await rerouteIfNeeded(document.uri, MatViewerProvider.viewType, webviewPanel)) {
                return;
            }

            const model = MatParser.parseFile(filePath, await FileUtils.getFileSize(filePath));
