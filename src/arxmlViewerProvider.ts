...
export class ArxmlViewerProvider implements vscode.CustomReadonlyEditorProvider {
    public static readonly viewType = 'omni-viewer.arxmlViewer';
    ...
    public async resolveCustomEditor(document: vscode.CustomDocument, webviewPanel: vscode.WebviewPanel): Promise<void> {
        ...
        registerRefreshableViewer(document.uri, ArxmlViewerProvider.viewType, webviewPanel, async () => {
        ...
        try {
            if (await rerouteIfNeeded(document.uri, ArxmlViewerProvider.viewType, webviewPanel)) {
                return;
            }

            const source = await fs.promises.readFile(filePath, 'utf8');
            const model = AutomotiveParsers.parseArxml(source, await FileUtils.getFileSize(filePath));
