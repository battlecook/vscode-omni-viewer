...
export class Db3ViewerProvider implements vscode.CustomReadonlyEditorProvider {
    public static readonly viewType = 'omni-viewer.db3Viewer';
    ...
    public async resolveCustomEditor(document: vscode.CustomDocument, webviewPanel: vscode.WebviewPanel): Promise<void> {
        ...
        registerRefreshableViewer(document.uri, Db3ViewerProvider.viewType, webviewPanel, async () => {
        ...
        try {
            if (await rerouteIfNeeded(document.uri, Db3ViewerProvider.viewType, webviewPanel)) {
                return;
            }

            const model = await AutomotiveParsers.parseDb3(filePath, await FileUtils.getFileSize(filePath));
